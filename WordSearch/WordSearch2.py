...
def go(board, word,pos,marked,i=0,j=0,count=0):
    ...
    right= go(board, word,pos+1,marked,i,j+1,count)
    if not right: 
        left=go(board, word,pos+1,marked,i,j-1,count)
        if not left:
            up=go(board, word,pos+1,marked,i+1,j,count) 
            if not up:
                down= go(board, word,pos+1,marked,i-1,j,count)       
