...
'''
def minimumCostPath(matrix,row=0,col=0,sum=0):
    ...
    if(row==len(matrix) or col==len(matrix[0])):
        ...
        return min(minimumCostPath(matrix,row+1,col,sum+matrix[row][col]),
                                     minimumCostPath(matrix,row,col+1,sum+matrix[row][col]))
